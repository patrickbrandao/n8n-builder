...
#====================================================================================================
    ...
    EDITOR_NAME=n8n-221-editor;
    WEBHOOK_NAME=n8n-221-webhook;
    MCPSERVER_NAME=n8n-221-mcp-server;
    WORKER_NAME=n8n-221-worker;
    RUNNER_NAME=n8n-221-runner;

    # Versao N8N
    SERVER_FQDN=$(hostname -f);
    FQDN_EDITOR=editor-221.$SERVER_FQDN;
    FQDN_WEBHOOKS=ws-221.$SERVER_FQDN;
    FQDN_MCP=mcp-221.$SERVER_FQDN;

    # Armazenamento
    DATADIR=/storage/n8n-221-data;
    NODEDIR=/storage/n8n-221-nodes;
    ...
    # Banco de dados
    DB_POSTGRESDB_DATABASE="n8n221";
    DB_POSTGRESDB_HOST="postgres-18";
    DB_POSTGRESDB_PORT="5432";
    DB_POSTGRESDB_USER="n8n221";
    DB_POSTGRESDB_PASSWORD="tulipasql";

    QUEUE_BULL_REDIS_HOST=redis-db
    QUEUE_BULL_REDIS_PORT=6379
    QUEUE_BULL_REDIS_DB=221
    QUEUE_BULL_PREFIX=bull221;
...
#====================================================================================================
    ...
    (
        ...
        echo "    \\";
    ) > /tmp/setup-n8n-221-01-defaults.txt;
    ...
    (
        ...
        echo "    \\";
        echo "    \\";
    ) > /tmp/setup-n8n-221-02-common.txt;
    ...
    (
        ...
        echo "    \\";
        echo "    \\";
    ) > /tmp/setup-n8n-221-04-external.txt;
    ...
    (
        ...
        echo "    \\";
    ) > /tmp/setup-n8n-221-06-database.txt;
    ...
    (
        ...
        echo "    \\";
    ) > /tmp/setup-n8n-221-07-editor.txt
    ...
    (
        ...
        echo "    \\";
    ) > /tmp/setup-n8n-221-08-redis.txt;
    ...
    (
        ...
        echo "    \\";
    ) > /tmp/setup-n8n-221-10-runners.txt;
    ...
    (
        ...
        echo "    \\";
    ) > /tmp/setup-n8n-221-11-worker-task-runner-mode.txt;
    ...
    (
        ...
        echo "    -e N8N_RUNNERS_AUTO_SHUTDOWN_TIMEOUT=$N8N_RUNNERS_AUTO_SHUTDOWN_TIMEOUT \\";
        echo "    \\";
    ) > /tmp/setup-n8n-221-12-task-runner-to-worker.txt;
...
#====================================================================================================
    ...
    (
        ...
        echo "    --ip=10.117.221.1 \\"
        echo "    --ip6=2001:db8:10:117::221:1 \\"
        echo "    --mac-address '02:cd:f2:21:00:01' \\";
        echo "    \\"

        cat /tmp/setup-n8n-221-01-defaults.txt;
        cat /tmp/setup-n8n-221-02-common.txt;
        cat /tmp/setup-n8n-221-03-smtp.txt;
        cat /tmp/setup-n8n-221-04-external.txt;
        cat /tmp/setup-n8n-221-05-exec.txt;
        cat /tmp/setup-n8n-221-06-database.txt;
        cat /tmp/setup-n8n-221-07-editor.txt;
        cat /tmp/setup-n8n-221-08-redis.txt;
        cat /tmp/setup-n8n-221-10-runners.txt;
        echo "    \\";
...
#====================================================================================================
    ...
    (
        ...
        echo "    --network network_public \\"
        echo "    --ip=10.117.221.2 \\"
        echo "    --ip6=2001:db8:10:117::221:2 \\"
        echo "    --mac-address '02:cd:f2:21:00:02' \\";
        echo "    \\";
...
#====================================================================================================
    ...
    (
        ...
        echo "    --network network_public \\"
        echo "    --ip=10.117.221.3 \\"
        echo "    --ip6=2001:db8:10:117::221:3 \\"
        echo "    --mac-address '02:cd:f2:21:00:03' \\";
        echo "    \\";
        ...
        echo;
    ) > /tmp/setup-n8n-221-mcp-server.sh;
...
#====================================================================================================
    ...
    (
        ...
        echo "    --network network_public \\"
        echo "    --ip=10.117.221.11 \\"
        echo "    --ip6=2001:db8:10:117::221:11 \\"
        echo "    --mac-address '02:cd:f2:21:00:11' \\";
        echo "    \\";
        ...
        cat /tmp/setup-n8n-221-07-editor.txt;
        cat /tmp/setup-n8n-221-08-redis.txt;
        cat /tmp/setup-n8n-221-11-worker-task-runner-mode.txt;
        echo "    \\";
        ...
        echo;
    ) > /tmp/setup-n8n-221-worker.sh;


# Task Runner
#====================================================================================================

    # Script do taskrunner
    (
        echo '#!/bin/sh';
        echo;
        echo "echo 'Parando: $RUNNER_NAME';";
        echo "docker stop $RUNNER_NAME 2>/dev/null;";
        echo "docker rm   $RUNNER_NAME 2>/dev/null;";
        echo;
        echo "echo 'Iniciando: $RUNNER_NAME';";
        echo "docker run \\";
        echo "    -d --restart=always \\";
        echo "    --name $RUNNER_NAME -h $RUNNER_NAME.intranet.br \\";
        echo "    \\";

        echo "    --network network_public \\"
        echo "    --ip=10.117.221.111 \\"
        echo "    --ip6=2001:db8:10:117::221:111 \\"
        echo "    --mac-address '02:cd:f2:21:01:11' \\";
        echo "    \\";
        ...
        cat /tmp/setup-n8n-221-07-editor.txt;
        cat /tmp/setup-n8n-221-08-redis.txt;
        cat /tmp/setup-n8n-221-12-task-runner-to-worker.txt;
        echo "    \\";
        ...
        echo "    \\";
        echo "    $RUNNERS_IMAGE javascript python;";
        echo;
    ) > /tmp/setup-n8n-221-runner.sh;
...
#====================================================================================================
    ...
    sh /tmp/setup-n8n-221-editor.sh;

    docker rm -f n8n-worker 2>/dev/null;
    sh /tmp/setup-n8n-221-worker.sh;

    docker rm -f n8n-webhook 2>/dev/null;
    sh /tmp/setup-n8n-221-webhook.sh

    docker rm -f n8n-mcp-server 2>/dev/null;
    sh /tmp/setup-n8n-221-mcp-server.sh;

    docker rm -f n8n-runner 2>/dev/null;
    sh /tmp/setup-n8n-221-runner.sh;
