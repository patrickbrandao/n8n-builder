...
#====================================================================================================
    ...
    EDITOR_NAME=n8n-231-editor;
    WEBHOOK_NAME=n8n-231-webhook;
    MCPSERVER_NAME=n8n-231-mcp-server;
    WORKER_NAME=n8n-231-worker;
    RUNNERJS_NAME=n8n-231-runner-js;
    RUNNERPY_NAME=n8n-231-runner-py;

    # Versao N8N
    SERVER_FQDN=$(hostname -f);
    FQDN_EDITOR=editor-231.$SERVER_FQDN;
    FQDN_WEBHOOKS=ws-231.$SERVER_FQDN;
    FQDN_MCP=mcp-231.$SERVER_FQDN;

    # Armazenamento
    DATADIR=/storage/n8n-231-data;
    NODEDIR=/storage/n8n-231-nodes;
    ...
    # Banco de dados
    DB_POSTGRESDB_DATABASE="n8n231";
    DB_POSTGRESDB_HOST="postgres-18";
    DB_POSTGRESDB_PORT="5432";
    DB_POSTGRESDB_USER="n8n231";
    DB_POSTGRESDB_PASSWORD="tulipasql";

    #QUEUE_BULL_REDIS_HOST=redis-db-e;
    QUEUE_BULL_REDIS_HOST=redis-db;
    QUEUE_BULL_REDIS_PORT=6379
    QUEUE_BULL_REDIS_DB=231;
    ...
    # TaskRunner - Upstream (do tr para o worker)
    N8N_RUNNERS_TASK_BROKER_URI="http://$WORKER_NAME:5679";
...
#====================================================================================================
    ...
    (
        ...
        echo "    \\";
    ) > /tmp/setup-n8n-231-01-defaults.txt;
    ...
    (
        ...
        echo "    \\";
        echo "    \\";
    ) > /tmp/setup-n8n-231-02-common.txt;
    ...
    (
        ...
        echo "    \\";
        echo "    \\";
    ) > /tmp/setup-n8n-231-04-external.txt;
    ...
    (
        ...
        echo "    \\";
    ) > /tmp/setup-n8n-231-06-database.txt;
    ...
    (
        ...
        echo "    \\";
    ) > /tmp/setup-n8n-231-07-editor.txt
    ...
    (
        ...
        echo "    \\";
    ) > /tmp/setup-n8n-231-08-redis.txt;

    # modo de runner de todos os containers (containers: editor, webhook e mcp)
    (
        echo "    \\";
        if [ "$N8N_RUNNERS_MODE" = "internal" ]; then
            # modo interno
            echo "    -e N8N_RUNNERS_ENABLED=$N8N_RUNNERS_ENABLED \\";
            echo "    -e N8N_RUNNERS_MODE=$N8N_RUNNERS_MODE \\";
        else
            # modo externo (container dedicado)
            echo "    -e N8N_RUNNERS_ENABLED=$N8N_RUNNERS_ENABLED \\";
            echo "    -e N8N_RUNNERS_MODE=$N8N_RUNNERS_MODE \\";
            echo "    -e N8N_RUNNERS_AUTH_TOKEN=$N8N_RUNNERS_AUTH_TOKEN \\";
        fi;
        echo "    \\";
    ) > /tmp/setup-n8n-231-10-runners.txt;

    # opcoes do worker para delegar ao task-runner (containers: worker)
    (
        echo "    \\";
        if [ "$N8N_RUNNERS_MODE" = "internal" ]; then
            # modo interno
            ...
            echo "    \\";
            echo "    -e N8N_RUNNERS_MODE=$N8N_RUNNERS_MODE \\";
            echo "    -e N8N_RUNNERS_BROKER_LISTEN_ADDRESS=$N8N_RUNNERS_BROKER_LISTEN_ADDRESS \\";
            # opcoes comuns entre worker e runner
            echo "    -e N8N_RUNNERS_AUTH_TOKEN=$N8N_RUNNERS_AUTH_TOKEN \\";
            echo "    -e N8N_NATIVE_PYTHON_RUNNER=$N8N_NATIVE_PYTHON_RUNNER \\";
            echo "    -e N8N_RUNNERS_MAX_CONCURRENCY=$N8N_RUNNERS_MAX_CONCURRENCY \\";
            echo "    -e N8N_RUNNERS_AUTO_SHUTDOWN_TIMEOUT=$N8N_RUNNERS_AUTO_SHUTDOWN_TIMEOUT \\";
        fi;
        echo "    \\";
    ) > /tmp/setup-n8n-231-11-worker-task-runner-mode.txt;

    # opcoes do task-runner para se conectar ao worker (containers: task-runner-js e task-runner-py)
    (
        echo "    \\";
        # runner aponta para o worker
        echo "    -e N8N_RUNNERS_TASK_BROKER_URI=$N8N_RUNNERS_TASK_BROKER_URI \\";
        echo "    \\";
        # opcoes comuns entre worker e runner
        echo "    -e N8N_RUNNERS_AUTH_TOKEN=$N8N_RUNNERS_AUTH_TOKEN \\";
        echo "    -e N8N_RUNNERS_MAX_CONCURRENCY=$N8N_RUNNERS_MAX_CONCURRENCY \\";
        echo "    -e N8N_RUNNERS_AUTO_SHUTDOWN_TIMEOUT=$N8N_RUNNERS_AUTO_SHUTDOWN_TIMEOUT \\";
        echo "    \\";
    ) > /tmp/setup-n8n-231-12-task-runner-to-worker.txt;
...
#====================================================================================================
    ...
    (
        ...
        echo "    --ip=10.117.231.1 \\"
        echo "    --ip6=2001:db8:10:117::231:1 \\"
        echo "    --mac-address '02:cd:f2:31:00:01' \\";
        echo "    \\"

        cat /tmp/setup-n8n-231-01-defaults.txt;
        cat /tmp/setup-n8n-231-02-common.txt;
        cat /tmp/setup-n8n-231-03-smtp.txt;
        cat /tmp/setup-n8n-231-04-external.txt;
        cat /tmp/setup-n8n-231-05-exec.txt;
        cat /tmp/setup-n8n-231-06-database.txt;
        cat /tmp/setup-n8n-231-07-editor.txt;
        cat /tmp/setup-n8n-231-08-redis.txt;
        cat /tmp/setup-n8n-231-10-runners.txt;
        echo "    \\";
...
#====================================================================================================
    ...
    (
        ...
        echo "    --network network_public \\"
        echo "    --ip=10.117.231.2 \\"
        echo "    --ip6=2001:db8:10:117::231:2 \\"
        echo "    --mac-address '02:cd:f2:31:00:02' \\";
        echo "    \\";
...
#====================================================================================================
    ...
    (
        ...
        echo "    --network network_public \\"
        echo "    --ip=10.117.231.3 \\"
        echo "    --ip6=2001:db8:10:117::231:3 \\"
        echo "    --mac-address '02:cd:f2:31:00:03' \\";
        echo "    \\";
        ...
        echo;
    ) > /tmp/setup-n8n-231-mcp-server.sh;
...
#====================================================================================================
    ...
    (
        ...
        echo "    --network network_public \\"
        echo "    --ip=10.117.231.11 \\"
        echo "    --ip6=2001:db8:10:117::231:11 \\"
        echo "    --mac-address '02:cd:f2:31:00:11' \\";
        echo "    \\";
        ...
        cat /tmp/setup-n8n-231-07-editor.txt;
        cat /tmp/setup-n8n-231-08-redis.txt;
        cat /tmp/setup-n8n-231-11-worker-task-runner-mode.txt;
        echo "    \\";
        ...
        echo;
    ) > /tmp/setup-n8n-231-worker.sh;


# Task Runner JavaScript
#====================================================================================================

    # Script do taskrunner JavaScript
    (
        echo '#!/bin/sh';
        echo;
        echo "echo 'Parando: $RUNNERJS_NAME';";
        echo "docker stop $RUNNERJS_NAME 2>/dev/null;";
        echo "docker rm   $RUNNERJS_NAME 2>/dev/null;";
        echo;
        echo "echo 'Iniciando: $RUNNERJS_NAME';";
        echo "docker run \\";
        echo "    -d --restart=always \\";
        echo "    --name $RUNNERJS_NAME -h $RUNNERJS_NAME.intranet.br \\";
        echo "    \\";

        echo "    --network network_public \\"
        echo "    --ip=10.117.231.111 \\"
        echo "    --ip6=2001:db8:10:117::231:111 \\"
        echo "    --mac-address '02:cd:f2:31:01:11' \\";
        echo "    \\";
        ...
        cat /tmp/setup-n8n-231-07-editor.txt;
        cat /tmp/setup-n8n-231-08-redis.txt;
        cat /tmp/setup-n8n-231-12-task-runner-to-worker.txt;
        echo "    \\";
        echo "    --mount type=bind,source=$DATADIR,destination=/home/node/.n8n,readonly=false \\";
        echo "    --mount type=bind,source=$NODEDIR,destination=/data/nodes,readonly=false \\";
        echo "    \\";
        echo "    --entrypoint /usr/local/bin/task-runner-launcher \\";
        echo "    \\";
        echo "    $RUNNERS_IMAGE javascript;";
        echo;
    ) > /tmp/setup-n8n-231-runner-js.sh;
...
#====================================================================================================
    ...
    # Script do taskrunner Python
    (
        echo '#!/bin/sh';
        echo;
        echo "echo 'Parando: $RUNNERPY_NAME';";
        echo "docker stop $RUNNERPY_NAME 2>/dev/null;";
        echo "docker rm   $RUNNERPY_NAME 2>/dev/null;";
        echo;
        echo "echo 'Iniciando: $RUNNERPY_NAME';";
        echo "docker run \\";
        echo "    -d --restart=always \\";
        echo "    --name $RUNNERPY_NAME -h $RUNNERPY_NAME.intranet.br \\";
        echo "    \\";

        echo "    --network network_public \\"
        echo "    --ip=10.117.231.231 \\"
        echo "    --ip6=2001:db8:10:117::231:231 \\"
        echo "    --mac-address '02:cd:f2:31:02:11' \\";
        echo "    \\";
        ...
        echo "    --entrypoint /usr/local/bin/task-runner-launcher \\";
        echo "    \\";
        echo "    $RUNNERS_IMAGE python;";
        echo;
    ) > /tmp/setup-n8n-231-runner-py.sh;
...
#====================================================================================================
    ...
    sh /tmp/setup-n8n-231-editor.sh;

    docker rm -f n8n-webhook 2>/dev/null;
    sh /tmp/setup-n8n-231-webhook.sh

    docker rm -f n8n-mcp-server 2>/dev/null;
    sh /tmp/setup-n8n-231-mcp-server.sh;

    docker rm -f n8n-worker 2>/dev/null;
    sh /tmp/setup-n8n-231-worker.sh;

    docker rm -f n8n-runner-js 2>/dev/null;
    sh /tmp/setup-n8n-231-runner-js.sh;

    docker rm -f n8n-runner-py 2>/dev/null;
    sh /tmp/setup-n8n-231-runner-py.sh;
