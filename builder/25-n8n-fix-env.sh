...
# Alterar variaveis de ambiente mudando valores padroes
    ...
    sed -i $SEDARG "/N8N_RUNNERS_MAX_CONCURRENCY/{n;s/10/128/;}" $TARGET;
    sed -i $SEDARG "/N8N_RUNNERS_MAX_CONCURRENCY/{n;s/5/128/;}" $TARGET;
    ...
    _backup "$TARGET";
    sed -i $SEDARG "/N8N_RUNNERS_GRANT_TOKEN/{n;s/''/'n8n'/;}" $TARGET;
    sed -i $SEDARG "/N8N_RUNNERS_MAX_CONCURRENCY/{n;s/10/128/;}" $TARGET;
    sed -i $SEDARG "/N8N_RUNNERS_TASK_TIMEOUT/{n;s/300/60/;}" $TARGET;
    ...
    # Colocar chave padrao para criptografia do n8n
    TARGET="./packages/@n8n/config/src/configs/instance-settings-config.ts";
    _backup "$TARGET";
    sed -i $SEDARG "/N8N_ENCRYPTION_KEY/{n;s/''/'n8n'/;}" $TARGET;
    _sed_diff;
